...
int main(int argc, char **argv) {
    ...
    ASSERT(sizeof(vec) == 24, "Fill in the correct value."); // macOS 输出24，之前一次在actions上运行的CI输出是40，取决于编译器、标准库实现、操作系统和架构（32位/64位）。
    {
        vec[20] = false;
    ...
    }
    {
        ...
        ref = false;
        ASSERT(!ref, "Fill in `ref` or `!ref`");
        // THINK: WHAT and WHY? // vec[30] 返回的是 std::vector<bool>::reference 类型的代理对象，而不是直接的 bool 值。当执行 ref = false 时，实际上是通过代理对象修改了 vec[30] 对应的位值。
