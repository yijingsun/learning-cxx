...
class DynFibonacci {
    size_t *cache;
    int cached;
    int capacity_; // 在类中存储数组大小，并在复制构造器中使用
...
public:
    ...
    // TODO: 实现复制构造器
    DynFibonacci(DynFibonacci const & a) : cache(new size_t[a.capacity_]),cached(a.cached),capacity_(a.capacity_) {
        // 深拷贝
        std::copy(a.cache, a.cache+capacity_, cache);
    };
